...
.PHONY: mss
mss: $(OUTPUT_DIR)/mss/MPFS_MSS_mss_cfg.xml

.PHONY: hss
hss: $(OUTPUT_DIR)/hss/build/hss-envm-wrapper.$(HSS_BOARD).hex

$(OUTPUT_DIR)/project.stamp $(OUTPUT_DIR)/project/$(PROJECT).prjx: $(OUTPUT_DIR)/mss/MPFS_MSS.cxz $(KRONUS_DIR)scripts/project.tcl $(SOURCES)
...
$(OUTPUT_DIR)/hss/build/hss-envm-wrapper.$(BOARD).hex: $(OUTPUT_DIR)/mss/MPFS_MSS_mss_cfg.xml sources/$(BOARD)/hss/def_config
	[ -d $(OUTPUT_DIR)/hss ] || git clone -b $(HSS_REVISION) --depth 1 $(HSS_REPOSITORY) $(OUTPUT_DIR)/hss
	cp $(OUTPUT_DIR)/mss/MPFS_MSS_mss_cfg.xml $(OUTPUT_DIR)/hss/boards/$(HSS_BOARD)/soc_fpga_design/xml/
	cp sources/$(BOARD)/hss/def_config $(OUTPUT_DIR)/hss/.config
	$(MAKE) -C $(OUTPUT_DIR)/hss BOARD=$(HSS_BOARD)

$(OUTPUT_DIR)/mss/MPFS_MSS_mss_cfg.xml $(OUTPUT_DIR)/mss/MPFS_MSS.cxz: sources/$(BOARD)/mss/MPFS_MSS.cfg
